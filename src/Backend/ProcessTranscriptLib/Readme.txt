...
Two other options for pre-processing transcripts are:

====== Option 1 =========
* Write a specification for the standardized format we expect.
* Allow a programmer in the community to create a program in whatever language he/she would like to use (Ruby, PHP, etc).
* Support the use of these languages on the server

======= Option 2 =========

Write a general conversion program that is based on "conversion definitions" input by a user.
The conversion definitions can be regular expressions that:
	Define a change to the text (delete unneeded text, reformat text)
	Define a capture of a value (name of speaker, date of meeting, etc)
